...
do
        echo "Test:$j Proc:$i" >> bdcats_vpic_1.1_redo.log 
	tproc=$i
	sizeperproc=0.25 #0.03125 # unit: million (particles)
	...
	echo 'filename:'$filename
        srun -n $tproc  ./VPIC $filename $poolname $CEPH_CONF $nparticle 0 >> bdcats_vpic_1.1_redo.log
	#clean the data
	wait
