...
/*  printf("Cached Blocks of %s: ",argv[1]); */
    printf ("page_size:%lu ",(unsigned long)page_size);
    for (page_index = 0; page_index <= file_stat.st_size/page_size; page_index++) {
        if (mincore_vec[page_index]&1) {
         // printf("page_index:%lu ", (unsigned long)page_index);
            nblocks++;
        }
    }
/*  printf("\n"); */
    printf("Number of cached blocks of %s: %ld\n", argv[1], nblocks );
    printf("Size of cached blocks of %s: %.2f MB\n",argv[1],nblocks*page_size/1024.0/1024.0);
    free(mincore_vec);
