...
void compound_read(char * src_file, hid_t dst_file, char * path_table, int write,int rank) {
    ...
    if ( H5Lexists( ifile, gt[0], lapl_id ) <= 0) {
        printf("group %s doens't exist\n", gt[0]);
        goto done;
    }
    igroup= H5Gopen(ifile,gt[0],H5P_DEFAULT);
    if(igroup<0) {
        printf("group '%s' open error rank: %d\n",gt[0],rank);
        goto done;
    }
    if ( H5Lexists( igroup, gt[1], lapl_id ) <= 0) {
        printf("table %s/%s doens't exist\n", gt[0], gt[1]);
        goto done;
    }
    ...
    if(ierr<0) {
        printf("ierr of get table info:%d in rank %d\n",ierr,rank);
        goto done;
    }
    ...
    if(strcmp(gt[1],tag1)==0) {
        ...
        /* printf("read number of records:%ld\n",nrecords); */
        if(write) {
